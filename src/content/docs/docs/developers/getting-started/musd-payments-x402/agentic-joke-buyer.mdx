...
- A running x402 seller to call. Either:
  - Your local seller from Quickstart Step 7 (`localhost:3000/joke`),
    or
...
The canonical `^2.11.0` pin lives on the
`feat/quickstart-humor-x402-2.11.0-canonical` branch — `main` of
`vativ/mezo-hack` still references the `v2.10.0-mezo.6` preview
tarball overrides, so cloning `main` alone will not match the
dependency setup these docs describe. Clone the feat branch
directly:

```bash
git clone \
  --branch feat/quickstart-humor-x402-2.11.0-canonical \
...
# Target URL. Pick one:
# RESOURCE_URL=http://localhost:3000/joke        # your local seller from the Quickstart
