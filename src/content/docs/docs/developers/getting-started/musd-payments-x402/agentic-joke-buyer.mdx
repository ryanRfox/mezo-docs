...
The client's `package.json` pulls `@x402/fetch`, `@x402/evm` (`^2.11.0`),
`viem`, and `dotenv` for you. Canonical `@x402/evm 2.11.0+` ships with
built-in Mezo support, so MUSD formatting and Mezo-specific constants
resolve correctly with no overrides. `@x402/fetch` is the same package
on every chain; Mezo support comes in through the `@x402/evm` scheme
plugin you register at runtime.
