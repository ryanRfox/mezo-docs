...
The Demo 3 code currently lives on the
`feat/agentic-3-trove-mandate-x402-2.11.0-canonical` branch of
`vativ/mezo-hack` — the canonical-`^2.11.0` sibling of
`feat/agentic-3-trove-mandate`. `main` of `vativ/mezo-hack` still
...
# Fresh clone:
git clone \
  --branch feat/agentic-3-trove-mandate-x402-2.11.0-canonical \
  https://github.com/vativ/mezo-hack.git
cd mezo-hack/apps/trove-advisor-mandate
pnpm install
```
...
git fetch && \
  git checkout feat/agentic-3-trove-mandate-x402-2.11.0-canonical
...
| `does not provide an export named 'DEFAULT_STABLECOINS'` | Installed `@x402/evm` is `2.10.x`, which lacks Mezo support | Pin `@x402/evm` to `^2.11.0` (and `@x402/paywall` if you also use it) in `package.json`; rerun `pnpm install` |
