...
You can swap either of the following tokens for gas:

- **MUSD**
- **MEZO**

Only tokens with available liquidity in a BTC pool are shown as options.
...
2. **Select a token**
   ...
   Choose which token (MUSD or MEZO) you'd like to swap for BTC. The modal will display the estimated cost in your selected token.

3. **Review the quote**

   The app quotes the amount of your selected token needed to receive exactly **0.0001 BTC**. The output amount is fixed so you always get a predictable gas amount regardless of token price.
